...
class ZundaOracleExtraMessage(ExtraMessage):
    ...
    def __init__(self, wake_word: str, api_key: str) -> None:
        ...
        try:
            self.faiss_indexes = FAISS.load_local('zunda_oracle.faiss', self.embedding)
        except:
    ...
    async def give_knowledge_command(self, inter: discord.Interaction, knowledge: str):
        ...
        if self.faiss_indexes is None:
            self.faiss_indexes = docsearch
        else:
            self.faiss_indexes.merge_from(docsearch)

        self.faiss_indexes.save_local('zunda_oracle.faiss')
